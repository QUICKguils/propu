...
def main():
    ...
    Pm = cst.mconv(115, "hp", "W")  # Mechanical power (Pm = Ps here)
    rho = cst.get_isa(0).rho
    D = cst.mconv(6, "ft", "m")

    ## 1. Propeller efficiency vs. airspeed
    ...
    D_range = np.linspace(0, 12 * cst.uconv("ft", "m"), 50)
    ...
    D_selected = np.array([3, 6, 9, 12]) * cst.uconv("ft", "m")
