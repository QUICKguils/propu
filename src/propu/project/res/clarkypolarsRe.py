"""
This python function is adapted from the MATLAB clarkypolarsRe(aoa, re)
function written by Thomas Lambert in 2023.

CLARKYPOLARSRE Returns the lift and drag coefficient for a Clark-Y airfoil
corresponding to one combination of angle of attack (in radians) and Reynolds
number for a Clark-Y airfoil.

This script works by interpolating hard-coded values for the polars. The
original values were obtained using XFoil for angles of attack between -20 and
20 degrees and Reynolds between 1e4 and 1e7 for a Mach number of 0. The polars
were then extended over the entire range of angles of attacks using empirical
formulas.

IMPORTANT:
The function works by loading the clarkypolars.mat file, which contains a polar
...
Note:
The extension of the polar was realised following the "Viterna" method. See
Viterna & Corrigan. 1982. "Fixed Pitch Rotor Performance of Large Horizontal
Axis Wind Turbines".
-----
Syntax:
cl, cd = clarkypolarsRe(aoa, re) returns the values for cl and cd of a Clark-Y
airfoil for the angle of attack aoa and reynolds number re.
...
----------------------------------------------------------------------------
This script was written for the 2024 BEMT project for the course of AERO0014
(Aerospace Propulsion) at the University of Liege. It is a gross over
simplification. Do not use it in any practical application.
----------------------------------------------------------------------------
